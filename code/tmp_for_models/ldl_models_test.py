...
repeat_genes = pd.read_csv("~/deepRIV/UKB/results/ldl/combine_p/cauchy_NL_results.csv")
hdl = pd.read_csv("/home/panwei/he000176/deepRIV/UKB/results/hdl/test40p_unrelated2/hdl_combined_results.csv")
repeat_genes['gene_ind'] = hdl.reset_index().set_index('gene_name').loc[repeat_genes.gene_name, 'gene_ind'].values
gene_name = repeat_genes.gene_name.iloc[file_num]
...
IVa_path = out_path + 'results/hdl/combine_p/repeat_ind/tmp_for_models/IVa_test_{}.txt'.format(file_num)
...
for j in range(num_params):
	...
	for k in range(j*num_repeats, (j+1)*num_repeats):
		...
		# save model
		if True:
			rsp1.save("/home/panwei/he000176/deepRIV/UKB/models/hdl/combine_p/tmp_for_models/model_test_{}_{}_{}".format(gene_ind, j, k))
		with open(debug_path, "a") as f:
			f.write("{}: Done\n".format(k))
		#
