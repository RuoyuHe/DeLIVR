...
repeat_genes = pd.read_csv("~/deepRIV/UKB/code/tmp_for_models/hdl_deepiv_genes.csv")
...
IVa_path = out_path + 'results/hdl/combine_p/repeat_deepiv/IV_{}.txt'.format(gene_ind)
...
IV_pval_global = np.zeros(num_params*num_repeats)
IV_pval_nonlinear = np.zeros(num_params*num_repeats)
IV_tval_global = np.zeros(num_params*num_repeats)
IV_tval_nonlinear = np.zeros(num_params*num_repeats)
...
for j in range(num_params):
	...
	for k in range(j*num_repeats, (j+1)*num_repeats):
		loss_path = out_path + 'results/hdl/combine_p/repeat_deepiv/loss_{}_{}_{}.txt'.format(gene_ind,j,k)
		...
		tmp = stage2(pred_x_test.reshape(-1,1))
		tmp = tf.reduce_mean(tf.reshape(tmp, (-1, training_params[j]['n_draws'])) ,axis = 1).numpy()
		IV_pval_global[k], IV_tval_global[k], IV_pval_nonlinear[k], IV_tval_nonlinear[k], nonlinear_coef[k], nonlinear_se[k] = stage2Tests(data['mean_x_test'], data['y_test'], tmp)
...
IVa_results = pd.concat([IVa_results, pd.DataFrame({"IV_pval_global":IV_pval_global, 
