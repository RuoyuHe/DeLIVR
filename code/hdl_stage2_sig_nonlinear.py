...
idx = np.where(hdl['IVa_pval_nonlinear'] <= 0.05/hdl.shape[0])[0]
indices = hdl['gene_ind'].iloc[idx].to_numpy()


gene_ind = indices[file_num]
...
# for gene_ind in sig_gene_ind:
	...
	# save model
	if True:
		rsp1.save("../models/hdl/sig_genes/v3/{}_{}".format(gene_ind, i))
	#
	i += 1
...
results.to_csv("~/deepRIV/UKB/results/stage2_results/sig_genes_{}.txt".format(gene_ind), index = False, sep = " ")
