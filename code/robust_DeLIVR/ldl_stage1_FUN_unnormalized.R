...
white_unrelated_keep = fread('~/deepRIV/UKB/data/white_unrelated_keep_ind.txt',
                             header = F)
exclude_ids = fread('~/deepRIV/UKB/data/exclude_ID.csv',
                    header = F)
ukb_pheno_all = fread('~/deepRIV/UKB/data/ukb_ldl.txt')
ukb_pheno_all = na.omit(ukb_pheno_all)
ukb_pheno_all = ukb_pheno_all %>% filter(!(f.eid %in% exclude_ids$V1))
ukb_pheno_all = ukb_pheno_all %>% filter(f.eid %in% white_unrelated_keep$V1)
...
stage1 <- function(gene_ind, 
  ...
  y_ukb_total = as.numeric(unlist(scale(ukb_pheno[,2])))
  ...
  FINAL_RESULT = list(gene=gene_name,
                      ...
                      stage2_theta = stage2_test$coefficients[2,1],
                      stage2_intercept = stage2_test$coefficients[1,1],
