...
IVa_path = out_path + 'results/ldl/robust_DeLIVR/IVa_{}.txt'.format(file_num)
debug_path = out_path + 'results/debug/ldl_robust_DeLIVR/{}.txt'.format(file_num)
...
for j in range(num_params):
	for k in range(j*num_repeats, (j+1)*num_repeats):
		...
		model_path = out_path + 'models/ldl/robust_DeLIVR/model_{}_{}_{}'.format(gene_ind,j,k)
		...
		with open(debug_path, "a") as f:
			f.write("{}: Done\n".format(k))
		#
		IVa_results = pd.DataFrame({"gene_name":gene_names[np.where(repeat_genes==gene_ind)[0][0]],
