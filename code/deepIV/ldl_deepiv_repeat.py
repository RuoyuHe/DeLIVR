...
repeat_genes = pd.read_csv("~/deepRIV/UKB/code/ldl/combine_p/deepIV/ldl_deepiv_genes.csv")
...
IVa_path = out_path + 'results/ldl/combine_p/repeat_deepiv/IV_{}.txt'.format(file_num)
debug_path = out_path + "results/debug/ldl_deepiv/{}.txt".format(file_num)
...
for j in range(num_params):
	...
	for k in range(j*num_repeats, (j+1)*num_repeats):
		loss_path = out_path + 'results/ldl/combine_p/repeat_deepiv/loss_{}_{}_{}.txt'.format(file_num,j,k)
		...
		# save model
		if True:
			stage2.save("/home/panwei/he000176/deepRIV/UKB/models/ldl/repeat_deepiv/model_{}_{}_{}".format(gene_ind, j, k))
		with open(debug_path, "a") as f:
			f.write("{}: Done\n".format(k))
		#
